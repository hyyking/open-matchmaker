...
from matchmaker.event.events import QueueEvent, DequeueEvent, ResultEvent, RoundStartEvent, RoundEndEvent
...
class QueueEventsTest(unittest.TestCase):
    ...
    def test_dequeue(self):
        evmap = EventMap.new()
        evmap.register(EqHandler(tag=3, key="team", expect=Team(team_id=69), kind=EventKind.DEQUEUE))
